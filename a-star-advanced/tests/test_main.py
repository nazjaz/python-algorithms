...
from src.main import AStar, GridGraph, Heuristic
...
class TestGridGraph:
    ...
    def test_custom_movement_costs(self):
        ...
        neighbors = graph.get_neighbors((2, 2))
        straight_neighbors = [n for n in neighbors if n[0] in [(2, 3), (2, 1), (3, 2), (1, 2)]]
        diagonal_neighbors = [n for n in neighbors if n[0] not in [(2, 3), (2, 1), (3, 2), (1, 2)]]
...
class TestAStar:
    ...
    def test_initialization_with_default_config(self):
        ...
        astar = AStar()
        assert astar.heuristic is not None
        assert isinstance(astar.allow_diagonal, bool)
    ...
    def test_initialization_with_custom_config(self):
        ...
        try:
            astar = AStar(config_path=config_path)
            assert astar.heuristic_name == "euclidean"
            assert astar.allow_diagonal is False
            assert astar.bidirectional is True
        finally:
    ...
    def test_simple_path(self):
        ...
        result = astar.search(graph, (0, 0), (4, 4))
    ...
    def test_path_with_obstacles(self):
        ...
        result = astar.search(graph, (0, 2), (4, 2))
    ...
    def test_same_start_and_goal(self):
        ...
        result = astar.search(graph, (2, 2), (2, 2))
    ...
    def test_bidirectional_vs_unidirectional(self):
        """Test that bidirectional explores fewer nodes."""
        grid = np.zeros((15, 15))
        graph = GridGraph(grid, allow_diagonal=True)
        ...
        config_path_uni = self.create_temp_config(config_uni)
        config_path_bi = self.create_temp_config(config_bi)

        try:
            astar_uni = AStar(config_path=config_path_uni)
            result_uni = astar_uni.search(graph, (0, 0), (14, 14))

            astar_bi = AStar(config_path=config_path_bi)
            result_bi = astar_bi.search(graph, (0, 0), (14, 14))

            assert result_uni["found"] is True
            assert result_bi["found"] is True
            assert result_uni["cost"] == result_bi["cost"]
        finally:
            Path(config_path_uni).unlink()
            Path(config_path_bi).unlink()
    ...
    def test_path_optimality(self):
        ...
        result = astar.search(graph, (0, 0), (4, 0))
    ...
    def test_config_file_not_found(self):
        ...
        with pytest.raises(FileNotFoundError):
            AStar(config_path="nonexistent.yaml")
